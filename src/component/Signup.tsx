...
const MultifieldInput = (props: any) => {
  const classes = useStyles();
  const { education, setEducation } = props;

  return (
    <>
      {education.map((obj: any, key: any) => (
        <Grid
          item
          container
...
const Login = () => {
  ...
  const [fileSelected, setFileSelected] = useState([]) // also tried <string | Blob>

  const handleImageChange = (e: any) => {
    console.log("e ------->", e)

      if (fileSelected && fileSelected.length) {
        setFileSelected(existing => existing.concat(Array.from(fileSelected)));
      }
      console.log("e ------->", e)
    }
